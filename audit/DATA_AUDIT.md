...
生成时间：2026-08-13 16:50:53 CST

## 总体结果

- 总记录：66,002 条；有效数值：64,106 条（97.13%）。
- 覆盖：8 类来源、18 个国家（地区）、61 个标准指标、3 类频率。
- 序列数：988；多源指标：24 个。
- 原始/直接标准化记录：60,824；派生对齐记录：5,178（7.85%）。
- 业务键重复：0；未登记指标：0。
...
- 仅覆盖单一国家（地区）的指标：16 个。
- 有效记录少于 100 条的来源：1 个。
- 完全无有效值的序列：28 条，不应计入有效覆盖。
- 当前年份之后的记录：1,360 条，需显式标记为预测值，不能与历史实绩混合。
- `status` 字段存在 5 种值，当前混合数据性质和来源状态码，需拆分语义。
